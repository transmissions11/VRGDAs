...
import {LogisticToLinearVRGDALib, LogisticToLinearVRGDAx} from "./composite/LogisticToLinearVRGDALib.sol";
...
contract LogisticToLinearNFT is ERC721 {
    ...
    // Note: instead of defining a single composite VRGDA, we could define two separate VRGDA's and
    // and use external conditions (sold units, time, etc.) to determine which VRGDA to use.
    // but we'll use the composite for the sake of example
    LogisticToLinearVRGDAx public vrgda; // The VRGDA used to price the NFT.
    ...
    {
        vrgda = LogisticToLinearVRGDALib.createLogisticToLinearVRGDA(
