...
contract TestContract is Test {
    ...
    uint256 constant max_days_since_start = 2;
    // step size (fraction of day); 1 means no fractional days
    uint256 constant step_day_fraction = 1;
    // multiple of per day target to check up to
    uint256 constant target_quantity_multiple = 2;
    ...
    function testFFICorrectnessOne() public {
        ...
        uint256 max_quantity = target_quantity_multiple * (uint256(perDay) / 1e18);
    ...
    function calculatePrice(
        ...
        inputs[10] = uint256(_timeSinceStart).toString();
        inputs[11] = "--sold";
        inputs[12] = uint256(_sold).toString();
